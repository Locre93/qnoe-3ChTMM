...
k = np.arange(500,3000,10) 										# Wavenumber [cm⁻¹] - 1/λ (4.497THz to 10.493THz)
ϵ_eff = Sheet.ModeEffectivePermittivity(k,Ef,ϵ)	

# Structure initialization
SHEET = Chunk(name="Suspended graphene",k=k,ϵ=ϵ_eff,length=2,units=nm)
BOUNDARY = Interface(name="Boundary",k=k).set(ReflectingInterface(k=k,ϕ=np.pi))		# Phese ϕ upon reflection

# System initialization
system = TMM_sSNOM_Simple([SHEET,BOUNDARY],position=100,site=1,units=nm)
